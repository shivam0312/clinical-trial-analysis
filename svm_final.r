...
intrain <- createDataPartition(data$V1,p=.0005,list = F)
...
svm.classifier <- svm(dtm,sample.data$V1,kernel="linear")
svm.pred <- predict(svm.classifier,modeldata[test, ])
svm.pred

# Confusion matrix
conf.mat <- table("Predictions" = svm.pred, Actual = cl[test])
